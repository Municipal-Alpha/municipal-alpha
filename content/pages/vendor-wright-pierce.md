Title: Wright-Pierce: Municipal Engineering Activity Across New England
Slug: vendors/wright-pierce
Status: hidden
Summary: Public profile compiled from municipal records. Wright-Pierce appears in 22+ New England municipalities across water, wastewater, and civil engineering work, with documented engagement spanning 2008-2026.
entity_name: Wright-Pierce
entity_type: engineering_firm
entity_hq: Topsham, ME
service_area: New England
schema_type: Organization

*Public profile compiled from publicly available municipal records: meeting minutes, agendas, check registers, RFPs, and engineering reports. Last updated 2026-05-11. [Request correction →](mailto:data@municipalalpha.com?subject=Wright-Pierce%20profile%20correction)*

## Wright-Pierce in New England

Wright-Pierce is a Topsham, Maine-headquartered engineering firm specializing in water, wastewater, and civil infrastructure for municipal clients across New England. Based on public municipal records across our coverage of 3,900+ US municipalities, the firm appears in **22 distinct New England municipalities** across Maine, New Hampshire, Connecticut, Vermont, and Massachusetts, with documented activity from 2008 through April 2026.

The deepest engagements are in Nashua, NH and Trumbull, CT, where Wright-Pierce shows up regularly in check-register records (vendor payment data) and meeting minutes across multiple boards.

| Metric | Value |
|---|---|
| New England municipalities with documented activity | 22 |
| Total document mentions in our corpus | 429 |
| Date range of documented activity | 2008-03-01 to 2026-04-22 |
| States with active engagement | ME, NH, CT, VT, MA |
| Primary practice areas (from document context) | Water, wastewater, stormwater, civil engineering |

## Where Wright-Pierce Works

The following municipalities have published documents mentioning Wright-Pierce in our coverage period. Sighting count reflects the number of distinct documents (meeting minutes, agendas, check registers, reports, RFPs) in which the firm is referenced.

| Municipality | State | Document mentions | Activity range |
|---|---|---:|---|
| Nashua | NH | 191 | 2013-02-15 to 2026-04-21 |
| Trumbull | CT | 176 | 2011-11-22 to 2025-02-27 |
| Montpelier | VT | 9 | 2024-11-15 to 2025-12-15 |
| Lewiston | ME | 7 | 2024-06-03 to 2026-01-12 |
| Wells | ME | 7 | 2019-08-06 to 2021-01-05 |
| Falmouth | ME | 6 | 2025-11-10 to 2026-03-03 |
| **Showing 6 of 22 New England municipalities** | | | **[Claim to unlock →](#claim-this-profile)** |

## Recent Project Activity (2025-2026)

Specific projects referenced in meeting minutes and agendas where Wright-Pierce is identified as the engineering consultant. Sourced from publicly published municipal records.

| Date | Municipality | Project / Context |
|---|---|---|
| 2026-04-22 | Cheshire, CT | I&I Study & Plant Capacity Evaluation; Denitrification Filter Facility Upgrade; Sewer Regulations Workshop (WPCA Meeting) |
| 2026-04-21 | Nashua, NH | Construction engineering services for Holman Stadium renovation and restoration (Board of Public Works) |
| 2026-04-01 | Nashua, NH | Contract awarded for Holman Stadium construction engineering (Finance Committee) |
| 2026-03-26 | Cheshire, CT | Memorandum re: Lakeside of Cheshire application; Sewer Regulations review (WPCA Minutes) |
| 2026-03-23 | Groton, CT | Bid review and award recommendation for Smith Lake Intake Screen project |
| 2026-03-16 | Keene, NH | Bypass pump line development for Martell Court Pump Station project (Conservation Commission) |
| 2026-03-16 | Torrington, CT | Harris Drive Pump Station Upgrade Project services |
| **Showing 7 of 19+ documented projects since 2008** | | **[Claim to unlock →](#claim-this-profile)** |

## Sample Payment Records (Public Check Registers)

The following payments appear in municipal check registers, public records of vendor payments. Reported as published; not aggregated by Municipal Alpha.

| Date | Municipality | Payment | Source |
|---|---|---:|---|
| 2025-10-15 | Nashua, NH | $214,346.58 across 6 checks | Record of Expenditures FY26 |
| 2025-12-17 | Nashua, NH | $79,201.80 across 4 checks | Record of Expenditures FY26 |
| 2026-01-07 | Nashua, NH | $76,786.29 across 6 checks | Record of Expenditures FY26 |
| 2025-11-19 | Nashua, NH | $66,074.13 across 7 checks | Record of Expenditures FY26 |
| **Showing 4 of 363 documented check-register entries** | | | **[Claim to unlock →](#claim-this-profile)** |

Comparable cadence is documented in Trumbull, CT (individual checks ranging $116 to $132,114 across FY24, FY25, FY26 warrants), Falmouth, ME, Chelmsford, MA, and Montpelier, VT.

## Methodology

This profile is compiled automatically from public municipal records. Municipal Alpha monitors 3,900+ US municipalities daily across 15+ government content management platforms. Vendor names are extracted from meeting minutes, agendas, check registers, RFPs, engineering reports, and bid tabulations, then aggregated by entity.

**What this profile is:** A factual aggregation of where Wright-Pierce has been documented as an engineering consultant in public municipal records. Every cited document is publicly available from the source municipality.

**What this profile is not:** A performance evaluation, a win-rate calculation, or an opinion on Wright-Pierce's services. All claims are factual citations from public records.

**Source data:** Specific source documents for any claim above are available on request. Each row in the activity table corresponds to a specific document published by the named municipality on or near the listed date.

Documents in this profile range from 2008 to April 2026. The corpus is updated continuously as new municipal records are published. Read more about [how Municipal Alpha works](/how-it-works/) and our [methodology](/methodology/).

## Claim This Profile

Are you Wright-Pierce? You can claim this profile to:

- Receive notifications when new municipal activity matching your firm is published
- Suggest corrections or clarifications to any data point above
- See expanded data not shown on the public page (additional municipalities, deeper project context, payment cadence analysis)
- Get access to **upcoming opportunities** in your service area not yet reflected here: RFPs, capital plan line items, and procurement activity in adjacent New England municipalities where your competitors are currently engaged

[**Claim this profile →**](mailto:vendors@municipalalpha.com?subject=Claim%20Wright-Pierce%20profile&body=I%20represent%20Wright-Pierce.%20Please%20share%20the%20expanded%20profile%20and%20discuss%20how%20to%20receive%20alerts%20for%20upcoming%20New%20England%20opportunities.)

[**Request correction →**](mailto:data@municipalalpha.com?subject=Wright-Pierce%20profile%20correction)

Corrections are processed within one business day. Disputed pages can be removed on the same day at the named firm's request. We publish municipal records, not opinions.

## About Municipal Alpha

Municipal Alpha builds a structured knowledge graph of US public municipal records, meeting minutes, agendas, check registers, RFPs, capital plans, and assessor data, across 3,900+ municipalities in 50 states. We provide alt-data products to financial firms, infrastructure analysts, and engineering consultancies. [Learn more →](/about/)

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Organization",
  "name": "Wright-Pierce",
  "url": "https://municipalalpha.com/vendors/wright-pierce/",
  "description": "Engineering firm specializing in water, wastewater, and civil infrastructure for municipal clients across New England. Profile compiled from public municipal records.",
  "address": {
    "@type": "PostalAddress",
    "addressLocality": "Topsham",
    "addressRegion": "ME",
    "addressCountry": "US"
  },
  "areaServed": [
    {"@type": "AdministrativeArea", "name": "Maine"},
    {"@type": "AdministrativeArea", "name": "New Hampshire"},
    {"@type": "AdministrativeArea", "name": "Connecticut"},
    {"@type": "AdministrativeArea", "name": "Vermont"},
    {"@type": "AdministrativeArea", "name": "Massachusetts"}
  ],
  "knowsAbout": [
    "Municipal engineering",
    "Water and wastewater infrastructure",
    "Stormwater management",
    "Civil engineering",
    "Pump station design",
    "Inflow and infiltration studies"
  ],
  "subjectOf": {
    "@type": "Dataset",
    "name": "Wright-Pierce Municipal Activity Profile",
    "description": "Aggregated public-record activity across 22 New England municipalities, 2008-2026",
    "creator": {
      "@type": "Organization",
      "name": "Municipal Alpha",
      "url": "https://municipalalpha.com"
    },
    "temporalCoverage": "2008-03/2026-04",
    "spatialCoverage": "New England, United States"
  }
}
</script>
